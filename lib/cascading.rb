module IncludeByDefault #:nodoc:

  # This module is a series of hacks designed to let you do the following with many-to-many scoped find operations:
  #
  #   Tag.find(8).posts.find(:all, :include => :tags)
  #
  # It is very messy and is basically a very roundabout way of removing <tt>:include</tt>s that cause problems,
  # rewriting them as <tt>:joins</tt>, adding appropriate column names to the SQL query, and notifying the
  # join dependency object that you want it to load the result set into objects so you don't ask the DB for it again.
  module Cascading
